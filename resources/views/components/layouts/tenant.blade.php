...
<header class="@container bg-gray-900">
    <nav class="bg-gray-900 {{ $navDisabled ? 'cursor-not-allowed pointer-events-none' : '' }}">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                <div>
                    <a href="{{ route('tenant.posts.index') }}" class="text-sm font-medium text-white uppercase">
                        {{ tenant('company') }}
                    </a>
                </div>
                    ...
                    <div class="ml-4 flex items-center md:ml-6">
                        @guest
                            ...
                            @if (Route::has('tenant.register'))
                                <x-button class="mt-1 ml-2" as="a" href="{{ route('tenant.register') }}">Register
                                </x-button>
                            @endif
                        @else
                            <div x-data="{ expanded: false }" @click.outside="expanded = false"
                                ...
                                <div>
                                    <button @click="expanded = !expanded"
                                        ...
                                        <img class="h-8 w-8 rounded-full" src="{{ auth()->user()->gravatar_url }}"
                                             alt="{{ auth()->user()->name }}">
                                    </button>
                                </div>
                                <div x-show="expanded" class="absolute right-0 mt-2 w-48 rounded-md shadow-lg"
                                     style="display: none;">
                                    <div class="py-1 rounded-md bg-white ring-1 ring-black ring-opacity-5">
                                        <a href="{{ route('tenant.settings.user') }}"
                                           class="block px-4 py-1 text-sm text-gray-700 hover:bg-gray-100">My
                                            account
                                        </a>
                                        @if (auth()->user()->isOwner())
                                            <a href="{{ route('tenant.settings.application') }}"
                                               class="block px-4 py-1 text-sm text-gray-700 hover:bg-gray-100">Application
                                                settings
                                            </a>
                                            <a href="{{ config('nova.path') }}"
                                               class="block px-4 py-1 text-sm text-gray-700 hover:bg-gray-100">Admin
                                                panel
                                            </a>
                                        @endif
    ...
    @if($title)
        <div class="bg-white border-b border-gray-200">
            <div class="max-w-7xl mx-auto py-2 px-4 sm:px-6 lg:px-8">
                <h1 class="uppercase text-lg font-semibold text-gray-900">
                    {{ $title }}
                </h1>
            </div>
        </div>
