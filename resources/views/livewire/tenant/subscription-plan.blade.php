        ...
        <div class="z-10 fixed bottom-0 inset-x-0 px-4 pb-4 sm:inset-0 sm:flex sm:items-center sm:justify-center">
            ...
            <div @mousedown.outside="cancelModalOpen = false" class="bg-white rounded-lg overflow-hidden shadow-xl transform sm:max-w-lg sm:w-full">
                ...
                <span class="flex w-full rounded-md shadow-sm sm:ml-3 sm:w-auto">
                    <button @click="@this.call('cancel', cancelationReason === 'Other' ? otherReason : cancelationReason); cancelModalOpen = false" type="button" class="w-full py-1 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-red-600 shadow-sm hover:bg-red-500 focus:outline-none focus:ring-blue focus:bg-red-500 active:bg-red-600 transition duration-150 ease-in-out">
                    Cancel subscription
                    </button>
                </span>
                <span class="mt-3 flex w-full rounded-md shadow-sm sm:mt-0 sm:w-auto">
                    <button @click="cancelModalOpen = false" type="button" class="w-full items-center py-1 px-4 border border-gray-300 text-sm font-medium rounded-md bg-white focus:outline-none focus:border-blue-300 focus:ring-blue active:text-gray-800 active:bg-gray-50 transition ease-in-out duration-150">
                    Close
                    </button>
                </span>
                </div>
