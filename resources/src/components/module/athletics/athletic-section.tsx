...
import {PaginationType, ATHLETIC_PRODUCT} from "~/lib/types";
...
const AthleticSection: Component<PROPS> = props => {
    ...
    return (<>
        <BaseDrawer side={'bottom'} contextId={'product-preview-1'}>
            ...
            <Grid class={'h-full w-full border-l border-gray-200'} cols={2} colsSm={2} colsMd={3} colsLg={5} colsXl={6}>
                <For each={data()}>
                    {(style: ATHLETIC_PRODUCT) => (

                        <AthleticListView href={`/team-athletics/${style.parent_sku}`} onClick={() => handler(style)} {...style} />
